...
class App extends Component {
  ...
  render() {
        const codas = this.state.codas.map(coda => {
          return <div className="col-lg-3 col-md-3" key={coda.id}>
          <div className="card box-shadow">
            ...
            <div className="card-body">
								<p className="card-text pressStartFont text-center">{coda.name}</p>
								<a href={coda.githubLink} className="btn btn-primary btn-block text-center" target="_blank" role="button" aria-pressed="true">GH Pages</a>
								<a href={coda.githubLink} className="btn btn-secondary btn-block btn-outline-secondary text-center" role="button" target="_blank" aria-pressed="true">BitBalloon</a>
							</div>
						</div>
					</div>
        })
